...
def main(argv) -> int:
    ...
    if etb_беды is not None:
        ...
        # РАНЬШЕ ЗДЕСЬ СТОЯЛ `return 0`, и это гасило весь остальной гейт.
        # Замер 25.09.2026 (аудит внешнего вклада, PR #27): страницы, в которых
        # встретился мост H62, получали зелёный, а сверка вызываемых методов с
        # отдаваемыми сервером и сверка числа аргументов НЕ ВЫПОЛНЯЛИСЬ вообще.
        # Плюс с 24.09 канон H106 объявил сам транспорт etb_* мёртвым: одна наша
        # проверка бракует страницу за эту строку, а эта по ней же выдавала зелёный.
        # Мост проверяем — и продолжаем остальными проверками, а не вместо них.
        print("  ~ внимание: etb_* — снятый протокол (H106); окно ОС его не слушает")
