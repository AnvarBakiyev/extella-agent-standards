...
Покупка себе делается отдельным флагом: после неё листинг больше нельзя удалить.
    ...
    python3 tools/deploy_app_from_modules.py apps/<slug> --выполнить --новая-версия   # снимок экспертов заново
...
def приёмка(listing_id: str, план: dict, карточка: dict, запрос, эксперты: list,
            опубликован_ок: bool = False) -> list:
    ...
    if сделан.get("published") is True and not опубликован_ок:
        беды.append("листинг оказался опубликованным — это не предрелиз")
    версии = сделан.get("versions") or [{}]
    версия = next((v for v in версии if v.get("id") == карточка.get("version_id")), версии[-1])
...
def выполнить(папка: pathlib.Path, запрос=None, купить: bool = False, план_только: bool = True,
             новая_версия: bool = False) -> dict:
    ...
    if план_только:
        return итог
    if карточка.get("listing_id") and not новая_версия:
        raise пк.Отказ(f"листинг уже создан: {карточка['listing_id']}. Новая версия — отдельное решение "
                       "(флаг --новая-версия): у опубликованного листинга она видна покупателям сразу (H20)")
    if карточка.get("listing_id"):
        # Новая версия несёт свежий снимок экспертов агента: после провижининга это единственный
        # способ донести эксперт до развёрнутых копий (замер на стенде VPS 05.09.2026).
        код, сырое = запрос(пк.ОС, f"/api/add-version-stream/{карточка['listing_id']}", поля=поля,
                            файлы={"page": страница, "icon": иконка})
        готово = пк.из_потока(сырое, "новая версия", код)
        карточка["version_id"] = готово.get("version_id")
        # Имя, описание, теги и иконка живут на листинге, а не на версии: add-version их не
        # трогает (замер 20.08.2026 в deploy_prerelease_claude). Обновляем отдельно, иначе
        # карточка в магазине отстаёт от плана.
        код, сырое = запрос(пк.ОС, f"/api/edit-listing/{карточка['listing_id']}",
                            поля={"name": поля["name"], "description": поля["description"], "tags": поля["tags"]},
                            файлы={"icon": иконка})
        if код != 200:
            raise пк.Отказ(f"карточка листинга не обновилась: HTTP {код} {str(сырое)[:160]}")
    else:
    ...
    итог["беды"] = приёмка(карточка["listing_id"], план, карточка, запрос, эксперты, опубликован_ок=новая_версия)
    if (купить or новая_версия) and карточка.get("version_id"):
        # Первая установка себе — на существующего агента; при новой версии своя копия
        # переустанавливается пустым телом, чтобы развёрнутый агент взял новый снимок.
        тело_покупки = {} if новая_версия else {"deploy_mode": "existing", "target_agent_id": план["agent_id"]}
        код, сырое = запрос(пк.ОС, f"/api/purchase-stream/{карточка['version_id']}", тело=тело_покупки)
        try:
...
def main(аргументы) -> int:
    ...
    try:
        итог = выполнить(папка, купить="--купить-себе" in аргументы, план_только="--выполнить" not in аргументы,
                         новая_версия="--новая-версия" in аргументы)
    except пк.Отказ as о:
        print(f"ОТКАЗ: {о}")
        return 1
    п = итог["поля"]
