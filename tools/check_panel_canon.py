...
РАДИУСЫ = {8, 12, 999}
# 1px и 2px в отступах — хайрлайны и оптические сдвиги, канон их отступами не считает.
ОТСТУПЫ = {0, 1, 2, 4, 8, 12, 16, 24, 32, 48}
# Токен со своим запасным стеком — «var(--mono,ui-monospace,Menlo,monospace)» —
# это тот же токен, а не чужой шрифт: сравниваем по началу имени, а не по целому.
...
# ✕ ✓ ✗ × — управляющие знаки кнопок, а не украшение: их канон не запрещает.
ЭМОДЗИ = re.compile('[\U0001F300-\U0001FAFF☀-➿️]')
УПРАВЛЯЮЩИЕ = set('✕✓✗×✔')


def текст_без_кода(s):
    """Оставляем только то, что человек видит: без script, style и комментариев.

    Комментарии автора — не интерфейс. Пока их считали текстом, гейт ругался на
    слово «Визард» в пояснении к коду, и это ложный запрет: его нельзя исполнить,
    не переписав объяснение, которое человеку и не показывают.
    """
...
def проверить(путь):
    ...
    # 2. Чужие семейства
    чужие = {}
    # Кавычки — часть значения, а не его граница. Прежний шаблон обрывался на первой же
    # кавычке, и объявление `--sans: 'Nunito', ui-sans-serif` захватывалось как один
    # пробел: канонный шрифт превращался в «чужой». Срабатывало только при пробеле после
    # двоеточия — поэтому evolution-console выглядела чистой, а profit-growth нет.
    # Гейт идёт в путь публикации, а там ложный запрет дороже пропущенного нарушения:
    # запрет, который нельзя исполнить, обходят целиком. Кавычки снимаем после захвата.
    for m in re.finditer(r'(?:font-family|--sans|--serif|--mono)\s*:\s*([^;\n}]+)', s):
        стек = m.group(1).strip().strip('"\'').strip()
        if not стек:
            continue
    ...
    for слово in МАШИННЫЕ:
        # Латинские термины ищем целым словом: без границ «PORT» находился внутри
        # «viewport», и панель получала нарушение на строке из <head>.
        шаблон = (r'\b' + re.escape(слово) + r'\b') if слово.isascii() else re.escape(слово)
        n = len(re.findall(шаблон, видимый, re.I))
        if n:
            машинные[слово] = n
    if машинные:
    ...
    # 11. Эмодзи
    эмодзи = [z for z in ЭМОДЗИ.findall(видимый) if z not in УПРАВЛЯЮЩИЕ]
    if эмодзи:
