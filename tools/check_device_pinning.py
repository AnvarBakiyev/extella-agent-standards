...
def проверить() -> int:
    ...
    for папка in ПАПКИ:
        ...
        for путь in корень.rglob("*"):
            ...
            # Файл может СОДЕРЖАТЬ образцы нарушений по делу — например, другой
            # инструмент, который эти нарушения ищет. Замер 26.09.2026: гейт покраснел
            # на `tools/audit_repo.py`, где образец закрепления лежит как признак для
            # поиска. Прятать образец ради зелени нельзя, поэтому файл объявляет себя
            # детектором явной меткой, а не попадает в список исключений гейта.
            if "ДЕТЕКТОР-ОБРАЗЦЫ" in текст:
                continue
            if чужой(путь, текст):
                continue
