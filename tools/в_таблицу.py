...
from формы import Отказ, проверить, пример  # noqa: E402
...
def клетки(д: dict, источник: str) -> list:
    ...
    if форма == "число":
        кл.append(_кл(0, 0, д["заголовок"], шапка=True))
        кл.append(_кл(1, 0, д["значение"]))
    elif форма == "список":
        # Колонки — в порядке первого появления: источник знает, что важнее.
        колонки = []
        for с in д["строки"]:
            for к in (с.get("поля") or {}):
                if к not in колонки:
                    колонки.append(к)
        кл.append(_кл(0, 0, "Название", шапка=True))
        for j, к in enumerate(колонки, 1):
            кл.append(_кл(0, j, к, шапка=True))
        for i, с in enumerate(д["строки"], 1):
            кл.append(_кл(i, 0, с["имя"]))
            for j, к in enumerate(колонки, 1):
                if к in (с.get("поля") or {}):
                    кл.append(_кл(i, j, с["поля"][к]))
    elif форма == "шаги":
        for j, з in enumerate(("№", "Шаг", "Пояснение")):
            кл.append(_кл(0, j, з, шапка=True))
        for i, ш in enumerate(д["шаги"], 1):
            кл.append(_кл(i, 0, i))
            кл.append(_кл(i, 1, ш["имя"]))
            if ш.get("подпись"):
                кл.append(_кл(i, 2, ш["подпись"]))
    elif форма == "связи":
        for j, з in enumerate(("От", "К", "Подпись")):
            кл.append(_кл(0, j, з, шапка=True))
        for i, с in enumerate(д["связи"], 1):
            кл.append(_кл(i, 0, с["от"]))
            кл.append(_кл(i, 1, с["к"]))
            if с.get("подпись"):
                кл.append(_кл(i, 2, с["подпись"]))
    elif форма == "документ":
        for j, з in enumerate(("Раздел", "Текст")):
            кл.append(_кл(0, j, з, шапка=True))
        for i, р in enumerate(д["разделы"], 1):
            кл.append(_кл(i, 0, р["имя"]))
            кл.append(_кл(i, 1, р["текст"]))
    else:
    ...
    след = (f"собрано агентом · источник: {источник} · {time.strftime('%d.%m.%Y %H:%M')}"
            + (f" · {д['подпись']}" if д.get("подпись") else ""))
...
def работа(д: dict, источник: str, порт: int, сухой: bool) -> int:
    print(f"\nФорма «{д['форма']}» · {д['заголовок']} · источник «{источник}» → лист таблицы")
    if сухой:
...
def selftest() -> int:
    ...
    кл = клетки(проверить(пример("список")), "проба")
    ...
    итог = сложить([человеческий, старый_агентский, чужой_агентский],
                   лист(проверить(пример("число")), "проба", 0), "проба")
    ...
    try:
        сложить([старый_агентский], лист(проверить(пример("число")), "проба", 0), "проба")
    ...
    с_числами = проверить({"форма": "список", "заголовок": "х",
                           "строки": [{"имя": "Алматы", "поля": {"долг": 4850000}}]})
