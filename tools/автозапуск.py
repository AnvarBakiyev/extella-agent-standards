...
def поставить(имя: str, команда: list[str], журнал: pathlib.Path | None = None) -> str:
    ...
    # «python3» в команде — не питон, а лотерея. Системный python3 на macOS
    # это 3.9, и служба падает на первой же современной строчке кода
    # («dict | None» → TypeError), причём падает УЖЕ ПОСЛЕ установки: снаружи
    # видно только «приложение не отвечает» (замер 23.08.2026, Документы).
    # Подставляем тот питон, которым запущены сами.
    команда = [питон() if ч in ("python3", "python") else ч for ч in команда]
...
def _самопроверка() -> int:
    ...
    if "python" in питон().lower() and "/" in питон():
        print(f"  ✓ питон для служб — полный путь, а не «python3»: {питон()}")
    elif питон().endswith("python3") or "python" in питон().lower():
