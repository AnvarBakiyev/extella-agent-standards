...
def приёмка(лид: str) -> dict:
    # Прогон иногда возвращает пустоту (нет png) или падает исключением на
    # холодном старте — перемежающийся флак. Скриншот = источник вердикта,
    # поэтому повторяем весь прогон целиком (свежий прокси/порт/браузер) с
    # растущей паузой, пока не получим непустой скриншот, до 4 попыток.
    import time
    сб, png, std, последняя_беда = None, None, None, "не удалось снять окно"
    for попытка in range(4):
        try:
            with ЗАМОК:
                сб = same_origin_probe.собрать(лид)
            if сб.get("ошибка"):
                последняя_беда = сб["ошибка"]
            else:
