...
def main(argv) -> int:
    ...
    for i, п in enumerate(приложения, 1):
        ...
        # Один повтор при падении прогона: холодный Chrome изредка вылетает по
        # таймауту (замер 06.09: Composio и MD Reader моргнули ⚪, при повторе
        # зелёные). Флак стенда — не провал приложения; серый ставим только если
        ...
        for попытка in (1, 2):
            try:
                ...
                в = {"цвет": "серый", "блок": False,
                     "жёсткие": [f"прогон упал: {str(беда)[:160]}"],
                     "мягкие": [], "проклик": []}
                if попытка == 2:
                    traceback.print_exc()
            if попытка == 1:
                print(f"    ⚪ повтор {имя} (первый прогон сорвался)", flush=True)
                time.sleep(5)
