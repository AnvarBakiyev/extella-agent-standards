...
ЧЕГО ЭТА ПРОВЕРКА НЕ ВИДИТ. Права агента — не единственная поверхность, с
которой достают токен аккаунта. Рабочий стол ОС выполняет чужой код рядом с
токеном ещё в трёх местах: действия правого клика, расширения из магазина и
заголовки веб-приложения с подстановкой. Ни одно из них не проходит через
список инструментов агента, поэтому здесь и не ловится. Разбор — раздел H33
в DEPLOY_REQUIREMENTS.md, карта возможностей — OS_CAPABILITIES.md.
