...
Правила этого инструмента, каждое куплено поломкой:
  ...
  * эксперт модуля копируется В СКОУП агента приложения (`global: false`), даже если он
    глобальный и виден агенту обычным чтением. Замер 05.09.2026 на стенде VPS: окно магазина
    зовёт `app-agent/run`, а тот ищет эксперт только в собственном скоупе агента и на
    глобальный отвечает «Expert not found». Копия, которая уже лежит в скоупе и совпадает с
    каноном, второй раз не пишется;
  * после записи эксперт перечитывается из скоупа агента: «успешно» от платформы фактом не
    является (канон платформы, правило 5);
...
def провести(папка: pathlib.Path, платформа: Платформа, реестр: dict, только_чтение: bool = False) -> dict:
    ...
    for имя in имена:
        # Только собственный скоуп агента (global: false): именно оттуда читает app-agent/run.
        в_скоупе = платформа.получить(имя, agent_id, False)
        канон = платформа.получить(имя, КАНОН_АГЕНТ, True)
        строка = {"expert": имя, "kind": "module", "visible_before": bool(в_скоупе), "action": "none",
                  "verified": bool(в_скоупе) and (not канон or в_скоупе.strip() == канон.strip())}
        if not канон and not в_скоупе:
            строка["action"] = "missing"
            строка["note_ru"] = "эксперта нет ни в скоупе агента приложения, ни в общем пространстве"
            строка["note_en"] = "the expert exists neither in the app agent scope nor globally"
        elif канон and в_скоупе.strip() != канон.strip() and not только_чтение:
            код, сырое = платформа.сохранить(имя, описание(канон) or имя, канон, agent_id)
            строка["action"] = "copied" if not в_скоупе else "refreshed"
            if код != 200:
        ...
        elif канон and в_скоупе.strip() != канон.strip():
            строка["note_ru"] = "копия в скоупе агента отстала от канона — запусти без --проверить"
            строка["note_en"] = "the copy in the agent scope is behind the canon — run without --проверить"
...
def selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if строки["toolkit_probe"]["action"] == "copied" and строки["toolkit_probe"]["verified"] \
            ...
            print("  ✓ эксперт модуля скопирован в скоуп агента приложения и сверен")
        else:
        ...
        # 2. Повторный прогон: копия уже в скоупе и совпадает с каноном — второй раз не пишется.
        было = len(записи)
        ...
        if {с["expert"]: с for с in отчёт2["experts"]}["toolkit_probe"]["action"] == "none" \
            ...
            print("  ✓ копия, совпадающая с каноном, второй раз не пишется")
        else:
            ошибки.append("копия в скоупе переписана без нужды")
        # 2а. Канон изменился — копия в скоупе обновляется.
        хранилище[("agent_extella_default", "toolkit_probe")] += "\n# v2"
        if {с["expert"]: с for с in провести(папка, платформа, реестр)["experts"]}["toolkit_probe"]["action"] == "refreshed":
            print("  ✓ отставшая от канона копия обновляется")
        else:
            ошибки.append("отставшая копия не обновлена")
