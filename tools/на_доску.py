...
def шапка(д: dict, источник: str, лев: float, верх: float) -> tuple:
    ...
    эл = [_подпись(источник, лев, верх, 900, д["заголовок"], 28)]
    y = верх + 44
    if д.get("подпись"):
        эл.append(_подпись(источник, лев, y, 900, перенос(д["подпись"], 90), 16, СЕРЫЙ))
        y += 26
...
def рисовать(д: dict, источник: str, лев: float, верх: float) -> list:
    ...
    if форма == "число":
        ...
        эл.append(_подпись(источник, лев + 28, y + 34, 500, str(д["значение"]), 40, БРОНЗА))
        return эл

    if форма == "список":
        for i, п in enumerate(д["строки"]):
        ...
        for i, п in enumerate(д["связи"]):
            ...
            if п.get("подпись"):
                эл.append(_подпись(источник, лев + 396, yy + 16, 96,
                                   перенос(п["подпись"], 14), 12, ПЕТРОЛЬ))
...
def selftest() -> int:
    ...
    эл = рисовать(проверить(пример("список")), "астра", 0, 0)
