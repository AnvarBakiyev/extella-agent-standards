...
def один_ли_продукт(slug: str) -> None:
    """Не собран ли этот же продукт вторым изданием — с другим установщиком.

    ЗАМЕР 27.08.2026. У Доски было два издания: `board` и `board-device`. Я
    чинил установщик в первом, а карточка магазина везла архив из второго —
    правок там не было ни одной. Внешне это выглядело как «исправления не
    помогают»: пакет собирался, версия росла, установка падала по-старому.

    Одинаковое имя продукта в двух папках — всегда ошибка, а не запас.
    """
    издание = КОРЕНЬ / "editions" / slug
    своё = издание / "listing.json"
    if not своё.exists():
        return
    try:
        имя = json.loads(своё.read_text()).get("name")
    except (json.JSONDecodeError, OSError):
        return
    двойники = []
    for чужое in (КОРЕНЬ / "editions").glob("*/listing.json"):
        if чужое.parent.name == slug:
            continue
        try:
            if json.loads(чужое.read_text()).get("name") == имя:
                двойники.append(чужое.parent.name)
        except (json.JSONDecodeError, OSError):
            continue
    if двойники:
        raise Отказ(f"продукт «{имя}» описан не только в «{slug}», но и в "
                    f"{', '.join(двойники)}. Одно из изданий уедет в магазин, "
                    f"а править вы будете другое. Оставьте одну папку")


def сверить_с_каноном(корзина: pathlib.Path) -> None:
    """Пакет собирается из рабочих копий — но канон живёт в `tools/`.

    ЗАМЕР 27.08.2026. Я добавил прокси латинские имена аргументов (чтобы
    установка проходила на Windows), поправил канон в `tools/cabinet_server.py`,
    собрал пакет — и установка упала на своей же машине. В пакет уехала рабочая
    копия из `~/extella-cabinet`, которая правки не знала. Снаружи это выглядело
    как новая поломка установщика, хотя установщик был прав, а копия отстала.

    Молчать об этом нельзя: покупатель получит старый код под новой версией.
    """
    расхождения = []
    for ф in корзина.rglob("*.py"):
        канон = КОРЕНЬ / "tools" / ф.name
        if канон.exists() and канон.read_bytes() != ф.read_bytes():
            расхождения.append(ф.relative_to(корзина))
    if расхождения:
        shutil.rmtree(корзина, ignore_errors=True)
        список = ", ".join(str(р) for р in расхождения)
        raise Отказ(f"в пакет едет код СТАРЕЕ канона: {список}. "
                    f"Рабочая копия отстала от tools/ — обновите её, "
                    f"иначе покупатель получит вчерашнюю версию под сегодняшним "
                    f"номером")


def собрать(slug: str, куда: pathlib.Path | None = None) -> pathlib.Path:
    один_ли_продукт(slug)
