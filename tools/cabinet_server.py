...
def сделать_обработчик(папка: pathlib.Path, файл_данных: pathlib.Path,
    ...
    class Обработчик(http.server.SimpleHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            with ЗАМОК:
                д = self._прочитать_всё()
                # СТИРАНИЕ РАБОТЫ ЧЕЛОВЕКА — ТОЛЬКО ЧЕРЕЗ КОПИЮ. В этом
                # хранилище лежит не настройка, а РАБОТА: листы таблицы, холст
                # доски. Приложение же в окне считает его обычным браузерным
                # хранилищем и вправе смести всё одним движением — при сбросе,
                # пересоздании, неудачном старте. Замер 23.08.2026: файл
                # Таблицы с десятью листами стал пустым за секунду, вернуть
                # было нечем. Два пути потери, и оба закрываем копией.
                опасно = ""
                if тело.get("очистить") and д:
                    опасно = "очистка всего хранилища"
                elif "ключ" in тело and тело.get("значение") is not None:
                    было = str(д.get(str(тело["ключ"])) or "")
                    стало = str(тело["значение"])
                    # Второй путь тише первого и оттого хуже: команды стереть
                    # нет, приложение просто кладёт поверх ПУСТОЙ лист. Снаружи
                    # это неотличимо от обычного сохранения.
                    if len(было) > 10000 and len(стало) < len(было) // 20:
                        опасно = (f"ключ «{тело['ключ']}» ужался с {len(было)} "
                                  f"до {len(стало)} знаков")
                if опасно:
                    # Имя копии — до миллисекунд, и существующую НЕ трогаем:
                    # две потери подряд укладываются в одну секунду, и копия с
                    # именем по секундам затирала предыдущую. Поймано пробой
                    # 23.08.2026: из двух копий на диске осталась одна, и
                    # именно та, где работы уже не было.
                    метка = int(time.time() * 1000)
                    копия = файл_данных.with_name(
                        файл_данных.stem + f".копия_{метка}.json")
                    while копия.exists():
                        метка += 1
                        копия = файл_данных.with_name(
                            файл_данных.stem + f".копия_{метка}.json")
                    try:
                        копия.write_text(json.dumps(д, ensure_ascii=False))
                        print(f"[хранилище] {опасно} — копия сохранена: "
                              f"{копия.name}", flush=True)
                    except OSError as е:
                        # Копию сохранить не смогли — НЕ стираем. Отказать
                        # приложению дешевле, чем молча снести работу человека.
                        return self._ответ(507, json.dumps(
                            {"ошибка": f"копия не сохранилась, стирать не буду: {е}"},
                            ensure_ascii=False).encode())
