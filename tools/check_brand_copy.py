...
# --- интерфейсные токены приложений (DESIGN_CODE.md в корне репозитория, автор Элла) ---
# Раскол закрыт 14.08.2026 замером: восемь цветов правила по дизайну отсутствовали
# в палитре брендбука, и любое приложение, собранное ПРАВИЛЬНО, падало на них.
# Брендбук описывает маркетинговые поверхности, правило Эллы — интерфейсы программ;
# палитра должна быть одна, поэтому интерфейсные токены включены сюда явно.
# Источник истины для интерфейсов — DESIGN_CODE.md в корне репозитория, не этот файл.
APP_UI = {"141414", "1E1E1E", "C9C3B8", "D7E0DC", "EFECE2", "F5F3EC", "FAF9F5", "FFFFFF"}
PALETTE = GOLD | LOGO_ONLY | PETROL | INK | SURFACE | STATUS | APP_UI
