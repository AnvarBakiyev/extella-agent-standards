...
def сохранить(папка: pathlib.Path, подпись: str) -> dict:
    ...
    if not итог.get("облако"):
        итог["что"] = (f"сохранено на диске ({итог['правок']} правок), "
                       f"но облака у папки нет — копии вне этого компьютера НЕТ")
        итог["без_облака"] = True
        return итог
...
def main() -> int:
    ...
    for папка in ПАПКИ:
        ...
        if not итог.get("есть"):
            continue
        знак = "✗" if итог.get("беда") else ("!" if итог.get("без_облака") else "✓")
        print(f"  {знак} {папка.name}: {итог.get('что')}")
        if итог.get("беда"):
            беды.append(папка.name)
        if итог.get("без_облака"):
            без_облака.append(папка.name)
    print()
    if а.посмотреть:
    ...
    if беды:
        ...
        return 1
    if без_облака:
        # ИТОГ НЕ ИМЕЕТ ПРАВА ОКРУГЛЯТЬ В СВОЮ ПОЛЬЗУ. Первая версия писала
        # «вся работа лежит в облаке», когда одна папка была сохранена только
        # на диске: человек уходил спокойным, а копии вне компьютера не было
        # (замер 23.08.2026 на своей же выдаче).
        print(f"  ИТОГ: сохранено, но БЕЗ ОБЛАКА осталось: {', '.join(без_облака)}. "
              f"Эти папки живут только на этом компьютере — пропадёт он, "
              f"пропадут и они.")
        return 0
