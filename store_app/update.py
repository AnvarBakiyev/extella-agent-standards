...
def токен() -> str:
    """Токен ВЛАДЕЛЬЦА ВИТРИНЫ, а не любой токен аккаунта.

    Витрина принадлежит хешу токена (H13): листинг, опубликованный из интерфейса,
    правится только тем же токеном. Служебный токен устройства получит 403, даже
    будучи токеном того же аккаунта. Поэтому первым читается отдельный файл,
    который владелец кладёт сам — так токен не проходит через чат.
    """
    витрина = pathlib.Path.home() / ".extella" / "os_token.txt"
    if витрина.exists():
        значение = витрина.read_text().strip()
        if значение:
            return значение
    ...
    отказ(
        ...
        "~/.extella/api_token.txt. Приложение Extella ключ на диск не кладёт — "
        "открой в нём Library → System → Tokens, создай токен витрины и сохрани "
        "в этот файл."
    )
...
def обновить():
    ...
    app_token_marker = 'var APP_TOKEN = /*APP_TOKEN*/"{{app_token}}";'
    if свежая.count(app_token_marker) != 1:
        беды.append("нет единственной служебной метки app_token")
    без_служебной_метки = свежая.replace(app_token_marker, "", 1)
    if "{" + "{" in без_служебной_метки:
        беды.append("в видимом тексте осталась метка подстановки — платформа покажет её значением")
    if беды:
    ...
    if код in (401, 403):
        отказ(
            "витрина не признала владельца. Листинг принадлежит тому токену, которым он "
            "опубликован (H13), а этот — другой. Положи токен, которым работаешь в ОС, "
            "в ~/.extella/os_token.txt (chmod 600) — скрипт прочитает его оттуда сам. "
            "Либо замени страницу вручную: Edit у карточки, поле Web page."
        )
    if код != 200:
