...
def dev_setup(app_name: str = "", version: str = "", token: str = "",
                agent_id: str = "", **extra) -> dict:
    """Installer for «Разработка на Extella». Picks the newest published version,
    ...
    ЗАЧЕМ ЭТОТ ФАЙЛ ПОЯВИЛСЯ. Замер витрины 18.09.2026: у продукта 13 загрузок — больше всех среди сломанных —
    архив и install.py на месте (install.py уже умеет платформы: есть sys.platform
    и ветка darwin), а поле installer_expert пустое. Ставить было нечем.
    ...
    LISTING = "880d12e4-f082-486e-b92a-57e4eb09866d"
    APP = (app_name or "").strip() or os.environ.get("EXTELLA_APP_NAME") or "Разработка на Extella"
    ...
    workdir = tempfile.mkdtemp(prefix="extella_dev_")
