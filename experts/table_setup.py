...
def table_setup(app_name: str = "", version: str = "", token: str = "",
                agent_id: str = "", **extra) -> dict:
    """Installer for «Table» (офисная Таблица). Picks the newest published version,
    downloads its archive from the Extella OS store, unpacks it and runs
    install.py from the archive root: lays out the atlas, starts its local
    server and reports the window address. Returns a report naming the machine
    it ran on. The platform passes app_name, version, token, agent_id; extra
    keyword arguments are accepted and ignored on purpose.

    ЗАЧЕМ ЭТОТ ФАЙЛ ПОЯВИЛСЯ. Замер витрины 19.09.2026: у четырёх карточек
    офисного набора — «Таблица», «Диаграммы», «Заметки», «Задачи и календарь» —
    нет НИ архива, НИ установщика. Это только страницы, которые стучатся в
    localhost, куда службу никто никогда не клал: тип карточки «extensions»
    вообще не умеет носить установщик. На Маке автора они работали, потому что
    он поднимал их руками, — у покупателя не работали никогда. «Таблица»
    переделывается первой: ей не нужен Docker, значит её путь проверяется
    целиком на чистом Windows-стенде.
    ...
    LISTING = "2ae60329-7f0b-4864-b595-366a6dcd28e3"
    APP = (app_name or "").strip() or os.environ.get("EXTELLA_APP_NAME") or "Table"
    ...
    def newest_version(auth):
        ...
        one. Ask the store for the top version instead. With LISTING empty
        (before the card exists) we deliberately skip this and take whatever
        the purchase gives."""
        if not LISTING:
            return ""
        try:
    ...
    workdir = tempfile.mkdtemp(prefix="extella_table_")
