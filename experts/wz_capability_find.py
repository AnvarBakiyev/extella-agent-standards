...
# description: Дверь агента к способностям Extella. По задаче словами возвращает 3-5 подходящих способностей из живого Capability Registry (289 записей: эксперты, автоматизации, блоки композитора, CSPL, установленное) с требованиями и границами из паспортов. Честно говорит «не нашёл» и «границы неизвестны» вместо выдумки. Ничего не запускает и не ставит — только находит.
#
# Копия платформенного эксперта в git (03.09.2026). До этого дня код жил только в общем
# пространстве платформы — то есть там, откуда его нельзя пересобрать. Правило реестра
# распространяется и на его дверь. Изменение 03.09: требования к машине (`needs`) берутся
# ещё и из реестра паспортов, куда сборщик кладёт их из MANIFEST.yaml модуля.
