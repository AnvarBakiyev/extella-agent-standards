...
def _след(поля):
    """Оставить отчёт об установке там, где его видно НЕ С ЭТОЙ машины.

    ЗАМЕР 27.08.2026. Владелец нажимал «Установить» на своей Windows, задача
    исполнялась и завершалась — а окно оставалось пустым. Узнать, что именно
    произошло на его машине, было НЕЧЕМ: вывод эксперта уходит в задачу, а
    списка задач у платформы нет — по номеру из подписи окна результат не
    достать. Разбор упирался в догадки.

    Поэтому установщик пишет короткий отчёт в общее хранилище: чем система,
    как звали машину, чем кончилось. Личных данных тут нет — имя компьютера и
    название системы; путей и токенов не пишем.
    """
    try:
        import platform, socket, time as _t
        запись = dict(поля)
        запись.update({"система": platform.system(),
                       "разрядность": platform.machine(),
                       "машина": socket.gethostname()[:40],
                       "когда": _t.strftime("%Y-%m-%d %H:%M:%S")})
        ядро = (os.environ.get("EXTELLA_API_BASE") or "https://api.extella.ai").rstrip("/")
        з = urllib.request.Request(
            ядро + "/api/kv/set",
            data=json.dumps({"key": "board_install_последний_отчёт",
                             "value": json.dumps(запись, ensure_ascii=False)}).encode(),
            headers={"X-Auth-Token": (os.environ.get("EXTELLA_AUTH_TOKEN") or ТОКЕН),
                     "X-Profile-Id": "default", "Content-Type": "application/json",
                     # БЕЗ ЭТОГО ЗАГОЛОВКА ЯДРО ОТКАЗЫВАЕТ. Первая версия следа
                     # его не слала, запись молча не создавалась — и средство
                     # разбора само оказалось слепым (замер 27.08.2026).
                     "X-Agent-Id": (os.environ.get("EXTELLA_AGENT_ID")
                                    or "agent_YbDQpKySopyi1ejHyQZuQ")})
        urllib.request.urlopen(з, timeout=60).read()
    except Exception:                                     # noqa: BLE001
        # След — вспомогательный. Если не записался, установка всё равно
        # должна идти своим ходом: молчащий отчёт лучше сорванной установки.
        pass
...
# ОТМЕТКА О СТАРТЕ. Без неё «эксперт упал» и «эксперт не запускался вовсе»
# выглядят одинаково — пустым окном, и разбор идёт вслепую.
_след({"этап": "начал работу", "версия": ВЕРСИЯ or "?",
       "токен_найден": bool(ТОКЕН)})
