...
def atlas_setup(app_name: str = "", version: str = "", token: str = "",
                agent_id: str = "", **extra) -> dict:
    """Installer for «Human Atlas». Picks the newest published version,
    ...
    ЗАЧЕМ ЭТОТ ФАЙЛ ПОЯВИЛСЯ. Замер витрины 18.09.2026: у Human Atlas есть
    страница, есть архив на 32 МБ и запрошено право device.run — но поле
    installer_expert пустое. Купить продукт можно, поставить нечем: платформе
    некого позвать. В архиве при этом install.py уже лежит (7,5 КБ) — не
    хватало ровно посредника между витриной и им. Тот же дефект найден ещё у
    трёх продуктов (Разработка на Extella, Personal Assistant, Lawyer).
    ...
    LISTING = "209c5932-32fa-4476-a9c5-83db74667e99"
    APP = (app_name or "").strip() or os.environ.get("EXTELLA_APP_NAME") or "Human Atlas"
    ...
    workdir = tempfile.mkdtemp(prefix="extella_atlas_")
