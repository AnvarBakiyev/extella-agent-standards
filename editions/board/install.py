...
def убить_на_порту(порт: int) -> None:
    """Остановить процесс, слушающий наш порт (Windows).

    Зовётся ТОЛЬКО после того, как проверено: на порту наша же Доска. Чужую
    программу так снимать нельзя — поэтому решение принимает `наше_ли`, а эта
    функция лишь исполняет.
    """
    try:
    ...
    for строка in (и.stdout or "").split("\n"):
        ...
        if len(куски) >= 5 and куски[-1].isdigit() and "LISTEN" in строка.upper():
            адрес = куски[1]
            if порт_из_адреса(адрес) == порт:
                subprocess.run(["taskkill", "/PID", куски[-1], "/F"],
                               capture_output=True)


def порт_из_адреса(адрес: str) -> int:
    """Порт из «127.0.0.1:34786» или «[::]:34786». Кривую строку молча
    пропускаем: разбор netstat не должен ронять установку."""
    try:
...
def снять_службу(имя: str = "") -> None:
    ...
    if с == "darwin":
        ...
        # ОСТАНОВИТЬ НАДО И ЗАДАЧУ, И САМ ПРОЦЕСС. Автозапуск у нас может жить
        # тремя способами (задача с папкой, задача плоская, ярлык в
        # автозагрузке), а сервер вдобавок поднимается напрямую при установке.
        # Замер 28.08.2026 на Windows владельца: переустановка через магазин
        # честно узнала свою же Доску на порту, попробовала снять её командой
        # планировщика — и не смогла, потому что процесс запускался мимо него.
        # Установка упиралась в «порт занят» на ровном месте.
        for задача in (f"Extella\\{имя}", f"Extella-{имя}"):
