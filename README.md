...
> [`AGENT_START.md`](AGENT_START.md) (9 КБ): там маршрут по задачам, десять ловушек и
> адресация правил по одному файлу. Свод `DEPLOY_REQUIREMENTS.md` — 369 КБ, веб-чтение
> его обрезает; поэтому правила разложены в [`rules/`](rules/INDEX.md) по 2–11 КБ каждое.
>
...
Скрипт находит ключ Extella, проверяет его **рукопожатием** с MCP и прописывает
сервер клиенту (Claude Code, Codex). **Действий человека — ноль.**

Ключ никто не выпускает вручную: он появляется на диске сам при первом запуске
приложения Extella (`~/.extella/os_token.txt`, права 600) либо создаётся человеком в `Library → System → Tokens`, и это тот же ключ,
который нужен MCP. Экрана «выдать токен» не существует, искать его не надо.
...
Единственный случай, когда нужен человек: Extella на этой машине ещё ни разу не
открывали. Тогда скрипт скажет это одной строкой — попроси открыть приложение
один раз и повтори команду.
