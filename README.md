...
а как продукт издание не доказано ни разу — мы своё же сняли. Поэтому правила про издания
лежат в `LAB.md`, а не в каноне: иначе следующий человек прочитает их как задание и
построит то, от чего мы отказались.
...
## Что мы делаем: ИИ-приложения

Ни «ИИ-агент», ни «приложение» не описывают то, что здесь строится, и путаница стоит нам
разговоров с клиентами. Слово, которым мы это называем:
...
| **Отдел** | несколько агентов одним листингом | «купи коммерческий блок», а не одного помощника |

**И это можно продавать.** Цена ставится при публикации; при покупке платформа списывает у
покупателя и **зачисляет автору**. Бесплатное тоже работает: наш гид стоит ноль и служит
входом для всех, кто строит.

Отсюда рамка, которую мы говорим наружу: **любой человек может собрать ИИ-приложение и
...
Сборка — одной командой: `python3 tools/deploy_edition.py путь/к/приложению`.
...
За один день 13.08.2026 это правило поймало три ложных вывода в наших же чатах:
...
> ⚠️ **Объяви права приложения (`app_scopes`) при публикации — иначе продукт молча мёртв.**
...
> пользователю. Пиши литералы HTML-мнемониками. `{{token}}` не запрашивай: он полноправный
...
**Порядок сдачи:** архив без секретов → права объявлены → **предрелиз** (виден только автору) →
